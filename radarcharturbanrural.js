...
var allAxis;
var radar_data_2
 function radarChartDrawUR(sdg_data, district_input) {
     ...
     var   subset_radar_1 = sdg_data.filter(function(el) {
                return el.District === district_input && (el.Demographic === "URBAN" || el.Demographic === "RURAL")});
     
     var   EICV_YEAR = d3.max(subset_radar_1.filter(function(el) { return el.Data === "EICV"}), function(d) {return d.Year;});
     
     var   DHS_YEAR = d3.max(subset_radar_1.filter(function(el) { return el.Data === "DHS"}), function(d) {return d.Year;});
     
     
     var    subset_radar_urban = subset_radar_1.filter(function(el) {
                return (el.Year  === EICV_YEAR || el.Year === DHS_YEAR) && el.Demographic === "URBAN"  ;});
     
    var    subset_radar_rural = subset_radar_1.filter(function(el) {
                return (el.Year  === EICV_YEAR || el.Year === DHS_YEAR) && el.Demographic === "RURAL"  ;});
     ...
     var color = d3.scaleOrdinal()
			...
			RadarChart2(".radarchartUR", radar_data_2, radarChartOptions); 
...
function RadarChart2(id, data, options) {
   ...
   var maxValue = Math.ceil((cfg.alternateVal)/10)*10 ;
	 ...
	 allAxis = (data[0].map(function(i, j){return i.Label}));	//Names of each axis
...
//			.attr("height", cfg.h + cfg.margin.top + cfg.margin.bottom)
	...
	var axisGrid = g.append("g").attr("class", "axisWrapper2");
	...
	var axis = axisGrid.selectAll(".axis2")
		.data(allAxis)
		.enter()
		.append("g")
		.attr("class", "axis2");
...
if(cfg.roundStrokes) {
	...
	var blobWrapper = g.selectAll(".radarWrapper2")
		.data(data)
		.enter().append("g")
	...
	//Append the circles
	blobWrapper.selectAll(".radarCircle2")
	...
	var blobCircleWrapper = g.selectAll(".radarCircleWrapper2")
		.data(data)
		.enter().append("g")
		.attr("class", "radarCircleWrapper2");
		
	//Append a set of invisible circles on top for the mouseover pop-up
	blobCircleWrapper.selectAll(".radarInvisibleCircle2")
		.data(function(d,i) { return d; })
		.enter().append("circle")
